...
\rfoot{Remade by \href{https://github.com/myhsia}{\ttfamily @myhsia}}
...
\begin{center}
  \begin{minipage}{.64\linewidth}
    ...
    右图这种\textbf{伤害大家视力}的行为，管理员看见后会直接撤回，丝毫不会手下留情.
    建议意识到后\textbf{主动撤回}.
  \end{minipage}
  \hspace*{\fill}
...
编译出错，效果不满意，想得到有针对性地帮助，请提供\textbf{最小工作示例代码}%
（\textbf Minimal \textbf Working \textbf Example, \textbf{MWE}），
完整 MWE 包括以下信息：
\begin{enumerate}
  \item 你使用的\textbf{操作系统}（Windows 7/10/11，macOS，Ubuntu，Deepin 等）.
  \item 安装的\textbf{发行版}
    （\hologo{TeX} Live 2025，Mac\hologo{TeX}，
      \hologo{MiKTeX}，\hologo{CTeX} 套装）.
  \item 你使用的\textbf{编辑器}
    （TeXstudio，Visual Studio Code，WinEdt，TeXshop，Overleaf）.
  \item 你执行的\textbf{编译命令}及\textbf{编译顺序}
  ...
  \begin{enumerate}
    ...
    \item 效果可截图，但\textbf{源代码不要截图！\textcolor{red}{不要拍屏幕！}}%
  ...
  \end{enumerate}
  \item 如实现某种需求，请\textbf{明确}提供该需求的\textbf{来源}，需求的必要环境（如要使用在列表环境内，必须使用 \file{xelatex} 编译等等）.
  \item 可以截图提供错误信息，奇怪的输出效果，可以通过 PS 等手段制作出你想要实现的效果图.
  \item 如果使用\textbf{模板}。请提供模板文件（\file{.cls}，\file{.sty}），或提供模板文件的获取方式.
  ...
  中.
  \item \textbf{如果无法主动提供信息，请明确说明并且及时回复潜在帮助者的提问.}
\end{enumerate}
